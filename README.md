...
│  │  │     │  ├─ segment/            # SQL 语义片段模型
│  │  │     │  │  ├─ dml/             # DML 片段（列、表达式、投影、排序、谓词、分页、集合、窗口等）
│  │  │     │  │  └─ generic/         # 通用片段（表、别名、所有者、注释、列、表达式、HINT 等）
│  │  │     │  ├─ statement/          # 语句层级抽象
│  │  │     │  ├─ util/               # 工具类（SQL 文本处理、日志封装等）
│  │  │     │  └─ value/              # 值节点与标识符封装
│  │  │     ├─ metadata/              # 元数据相关
│  │  │     │  ├─ model/              # 元数据模型
│  │  │     │  │  ├─ core/            # 核心元数据模型
│  │  │     │  │  ├─ reference/       # 引用模型（表引用、列引用）
│  │  │     │  │  ├─ field/           # 字段元数据
│  │  │     │  │  ├─ join/            # JOIN 关系
│  │  │     │  │  ├─ clause/          # 子句元数据
│  │  │     │  │  ├─ feature/         # 特性元数据（CTE、HINT、LATERAL VIEW、集合操作）
│  │  │     │  │  ├─ comment/         # 注释元数据
│  │  │     │  │  ├─ window/          # 窗口函数元数据
│  │  │     │  │  └─ function/        # 函数调用元数据
│  │  │     │  └─ extractor/          # 元数据抽取器
│  │  │     ├─ validation/            # 校验器 & 错误模型
│  │  │     └─ visitor/               # AST Visitor
│  │  │        ├─ core/               # AST Visitor 接口与共用抽象
│  │  │        └─ odps/               # ODPS 专用访问器与语义扩展
│  │  └─ resources/                   # 资源文件
...
| **元数据抽取器** | `metadata/extractor/OdpsSQLMetadataExtractor` | 遍历 AST，输出表、JOIN、字段血缘、注释、窗口函数等元数据信息 |
| **校验器** | `validation/OdpsSQLValidator` | 封装解析、错误监听、AST 构造与元数据抽取，提供 `validate(String sql)` |
| **校验结果** | `validation/OdpsSQLValidationResult` | 校验结果对象，包含 `formatSummary()` 生成用户友好摘要 |
...
- **`dml/column/`**：列片段（`ColumnSegment`）
- **`dml/expr/`**：表达式片段（`ExpressionSegment`、`BinaryOperationExpression`、`NotExpression`、`WindowFunctionSegment` 等）
- **`dml/item/`**：投影项片段（`ProjectionSegment`、`ColumnProjectionSegment`、`ExpressionProjectionSegment`）
...
- **`generic/`**：通用片段（`AliasSegment`、`OwnerSegment`、`TableSegment`、`CommentSegment`、`HintSegment` 等）
...
# 运行特定测试类
mvn test -Dtest=OdpsSQLValidatorTest
...
if (result.isValid()) {
    OdpsSQLMetadata metadata = result.getMetadata().orElseThrow(IllegalStateException::new);
...
if (result.isValid()) {
    ...
    metadata.getTables().forEach(table -> {
        if (table.getComment() != null) {
            System.out.println("表 " + table.getQualifiedName() + " 的注释: " + table.getComment());
        }
    });
    ...
    metadata.getFields().forEach(field -> {
        if (field.getComment() != null) {
            System.out.println("字段 " + field.getExpression() + " 的注释: " + field.getComment());
        }
    });
}
```
...
项目包含 **4 个测试类**，共 **69 个测试方法**：

| 测试类 | 测试方法数 | 主要测试内容 |
| --- | --- | --- |
| `OdpsSQLValidatorTest` | 16 | SQL 校验、元数据提取、复杂 SQL 场景 |
| `CommentExtractionTest` | 13 | 注释提取功能（表、字段、子句、语句级别） |
| `OdpsAstBuilderVisitorTest` | 25 | AST 构建（DISTINCT、JOIN、LIMIT、CTE、HINT、WINDOW、LATERAL VIEW、集合操作、子查询、ORDER BY、注释） |
...
函数调用提取功能已完整实现，支持以下特性：

### 支持的函数类型

- **聚合函数**：`COUNT`、`SUM`、`AVG`、`MIN`、`MAX`、`GROUP_CONCAT`、`STDDEV`、`VARIANCE`、`COLLECT_LIST`、`COLLECT_SET` 等
- **字符串函数**：`SUBSTRING`、`CONCAT`、`UPPER`、`LOWER`、`TRIM`、`REPLACE`、`LENGTH` 等
- **数学函数**：`ABS`、`ROUND`、`FLOOR`、`CEIL`、`POWER`、`SQRT`、`MOD` 等
- **日期时间函数**：`DATE_FORMAT`、`YEAR`、`MONTH`、`DAY`、`NOW`、`CURRENT_DATE`、`CURRENT_TIMESTAMP` 等
- **类型转换函数**：`CAST`、`CONVERT`、`TO_NUMBER`、`TO_STRING`、`TO_DATE` 等
- **条件函数**：`COALESCE`、`NULLIF`、`IFNULL`、`NVL`、`DECODE` 等
- **窗口函数**：`ROW_NUMBER`、`RANK`、`DENSE_RANK`、`SUM() OVER` 等（通过窗口函数元数据单独提取）

### 提取的信息

- **函数名称**：函数的完整名称（大写形式）
- **函数类型**：根据函数名称自动分类为上述类型之一
- **函数参数**：参数表达式文本列表和参数数量
- **调用位置**：函数在哪个子句中被调用（SELECT、WHERE、HAVING、ORDER BY 等）
- **函数别名**：如果函数调用有别名（如 `COUNT(id) AS cnt`）
- **DISTINCT 标识**：是否为 DISTINCT 聚合（如 `COUNT(DISTINCT col)`）
- **嵌套函数支持**：支持嵌套函数调用的完整提取（如 `SUM(ROUND(price, 2))`）