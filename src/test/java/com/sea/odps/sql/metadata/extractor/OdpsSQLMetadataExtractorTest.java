...
import com.sea.odps.sql.validation.OdpsSQLValidationResult;
import com.sea.odps.sql.validation.OdpsSQLValidator;
...
public class OdpsSQLMetadataExtractorTest extends TestCase {
  ...
  public void testAggregateFunctionExtraction() {
    ...
    assertTrue(
        "应该有 COUNT 函数",
        aggregateFunctions.stream().anyMatch(f -> "COUNT".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 SUM 函数",
        aggregateFunctions.stream().anyMatch(f -> "SUM".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 AVG 函数",
        aggregateFunctions.stream().anyMatch(f -> "AVG".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 MIN 函数",
        aggregateFunctions.stream().anyMatch(f -> "MIN".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 MAX 函数",
        aggregateFunctions.stream().anyMatch(f -> "MAX".equalsIgnoreCase(f.getFunctionName())));
  }
  ...
  public void testStringFunctionExtraction() {
    ...
    assertTrue(
        "应该有 SUBSTRING 函数",
        stringFunctions.stream().anyMatch(f -> "SUBSTRING".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 UPPER 函数",
        stringFunctions.stream().anyMatch(f -> "UPPER".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 LOWER 函数",
        stringFunctions.stream().anyMatch(f -> "LOWER".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 CONCAT 函数",
        stringFunctions.stream().anyMatch(f -> "CONCAT".equalsIgnoreCase(f.getFunctionName())));
  }
  ...
  public void testMathFunctionExtraction() {
    ...
    assertTrue(
        "应该有 ROUND 函数",
        mathFunctions.stream().anyMatch(f -> "ROUND".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 FLOOR 函数",
        mathFunctions.stream().anyMatch(f -> "FLOOR".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 CEIL 函数",
  ...
  public void testDateTimeFunctionExtraction() {
    ...
    assertTrue(
        "应该有 YEAR 函数",
        dateTimeFunctions.stream().anyMatch(f -> "YEAR".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 MONTH 函数",
        dateTimeFunctions.stream().anyMatch(f -> "MONTH".equalsIgnoreCase(f.getFunctionName())));
  }
  ...
  public void testCastFunctionExtraction() {
    ...
    boolean hasConvert =
        castFunctions.stream().anyMatch(f -> "CONVERT".equalsIgnoreCase(f.getFunctionName()));
    boolean hasToNumber =
        castFunctions.stream().anyMatch(f -> "TO_NUMBER".equalsIgnoreCase(f.getFunctionName()));
    boolean hasToString =
        castFunctions.stream().anyMatch(f -> "TO_STRING".equalsIgnoreCase(f.getFunctionName()));
    boolean hasToDate =
        castFunctions.stream().anyMatch(f -> "TO_DATE".equalsIgnoreCase(f.getFunctionName()));
  ...
  public void testConditionalFunctionExtraction() {
    ...
    // 至少应该有部分条件函数被识别
    assertTrue("应该有条件函数，实际数量: " + conditionalFunctions.size(), conditionalFunctions.size() >= 2);
    ...
    boolean hasNullif =
        conditionalFunctions.stream().anyMatch(f -> "NULLIF".equalsIgnoreCase(f.getFunctionName()));
    boolean hasIfnull =
        conditionalFunctions.stream().anyMatch(f -> "IFNULL".equalsIgnoreCase(f.getFunctionName()));
    boolean hasNvl =
        conditionalFunctions.stream().anyMatch(f -> "NVL".equalsIgnoreCase(f.getFunctionName()));
    boolean hasDecode =
        conditionalFunctions.stream().anyMatch(f -> "DECODE".equalsIgnoreCase(f.getFunctionName()));
  ...
  /**
   * 测试窗口函数识别。 注意：窗口函数在 extractWindowFunction 中被单独处理，可能不会出现在 functionCalls 中。 但应该出现在 windowFunctions
   * 元数据中。
   */
  @Test
  public void testWindowFunctionExtraction() {
    ...
    List<FunctionCallMetadata> windowFunctionCalls =
        functionCalls.stream().filter(f -> f.isWindowFunction()).collect(Collectors.toList());
  ...
  public void testNestedFunctionCalls() {
    ...
    assertTrue(
        "应该有 ROUND 函数",
        functionCalls.stream().anyMatch(f -> "ROUND".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 COALESCE 函数",
        functionCalls.stream().anyMatch(f -> "COALESCE".equalsIgnoreCase(f.getFunctionName())));
    assertTrue(
        "应该有 ABS 函数",
  ...
  public void testMixedFunctionTypes() {
    ...
    assertTrue(
        "应该有聚合函数",
        functionCalls.stream().anyMatch(f -> f.getFunctionType() == FunctionType.AGGREGATE));
    assertTrue(
        "应该有字符串函数",
        functionCalls.stream().anyMatch(f -> f.getFunctionType() == FunctionType.STRING));
    assertTrue(
        "应该有数学函数", functionCalls.stream().anyMatch(f -> f.getFunctionType() == FunctionType.MATH));
    assertTrue(
        "应该有日期时间函数",
        functionCalls.stream().anyMatch(f -> f.getFunctionType() == FunctionType.DATE_TIME));
  ...
  public void testFieldCategoryForAggregateFunctions() {
    ...
    assertEquals("cnt 字段应该是 AGGREGATE 类型", FieldCategory.AGGREGATE, cntField.get().getCategory());
    ...
    assertEquals(
        "total 字段应该是 AGGREGATE 类型", FieldCategory.AGGREGATE, totalField.get().getCategory());
    ...
    assertEquals(
        "avg_price 字段应该是 AGGREGATE 类型", FieldCategory.AGGREGATE, avgPriceField.get().getCategory());
  }
  ...
  public void testFunctionAliasExtraction() {
    ...
    Optional<FunctionCallMetadata> sumFunc =
        functionCalls.stream().filter(f -> "SUM".equalsIgnoreCase(f.getFunctionName())).findFirst();
