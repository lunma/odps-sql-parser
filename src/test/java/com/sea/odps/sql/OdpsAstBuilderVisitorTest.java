...
public class OdpsAstBuilderVisitorTest extends TestCase {
  ...
  public void testLimitClause() {
    ...
    assertTrue(
        "offset 应该是数字字面量", limit1.getOffset().get() instanceof NumberLiteralPaginationValueSegment);
    assertTrue(
        "rowCount 应该是数字字面量",
  ...
  public void testWithCTE() {
    ...
    assertEquals(
        "应该有 1 个 CTE", 1, selectStatement.getWith().get().getCommonTableExpressions().size());
  }
  ...
  public void testLateralView() {
    String sql = "select user_id, item from ods.orders " + "lateral view explode(items) t as item;";
    ...
    assertEquals("应该有 1 个列别名", 1, selectStatement.getLateralView().get().getColumnAliases().size());
  }
  ...
  public void testCombineOperations() {
    ...
    // UNION ALL
    String sql2 = "select id, name from ods.table_a union all select id, name from ods.table_b;";
    OdpsSQLSelectStatement stmt2 = parseSQL(sql2);
    ...
    // INTERSECT
    String sql3 = "select id, name from ods.table_a intersect select id, name from ods.table_b;";
    OdpsSQLSelectStatement stmt3 = parseSQL(sql3);
  ...
  public void testHavingComment() {
    ...
    assertTrue(
        "注释应该包含'过滤条件'", selectStatement.getHaving().get().getComment().getText().contains("过滤条件"));
  }
  ...
  public void testLimitComment() {
    ...
    assertTrue(
        "注释应该包含'限制10条'", selectStatement.getLimit().getComment().getText().contains("限制10条"));
  }
