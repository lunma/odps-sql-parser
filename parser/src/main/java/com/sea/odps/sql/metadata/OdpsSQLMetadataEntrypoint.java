...
import com.sea.odps.sql.error.OdpsSyntaxErrorListener;
 ...
 * </ul>
 */
public class OdpsSQLMetadataEntrypoint {

    private static final Logger log = LoggerFactory.getLogger(OdpsSQLMetadataEntrypoint.class);

    private final OdpsSQLMetadataExtractor metadataExtractor;

    /** 构造函数，使用默认的元数据提取器。 */
    public OdpsSQLMetadataEntrypoint() {
     ...
     */
    public OdpsSQLMetadataEntrypoint(final OdpsSQLMetadataExtractor metadataExtractor) {
     ...
     */
    public OdpsSQLMetadataResult result(final String sql) {
        Objects.requireNonNull(sql, "sql must not be null");

        // 检查空 SQL
        String trimmedSql = sql.trim();
        if (trimmedSql.isEmpty()) {
            OdpsSQLMetadataError error =
                    new OdpsSQLMetadataError(1, 0, "SQL statement cannot be empty");
            return OdpsSQLMetadataResult.unsupported(Collections.singletonList(error));
        }
        ...
        try {
            ...
            // 捕获明确的不支持操作异常
            String message =
                    String.format(Locale.ROOT, "Unsupported operation: %s", ex.getMessage());
            log.debug("Unsupported operation during AST building: {}", ex.getMessage(), ex);
            OdpsSQLMetadataError error = new OdpsSQLMetadataError(0, 0, message);
            return OdpsSQLMetadataResult.unsupported(Collections.singletonList(error));
        ...
        if (errorListener.hasErrors()) {
            List<OdpsSQLMetadataError> errors = errorListener.getErrors();
            log.debug("Syntax errors found: {}", errors.size());
            return OdpsSQLMetadataResult.withErrors(errors);
        }
        ...
        try {
            ...
            return OdpsSQLMetadataResult.success(metadata);
        } catch (Exception ex) {
            // 元数据提取过程中的错误
            String message =
                    String.format(Locale.ROOT, "Metadata extraction error: %s", ex.getMessage());
            log.warn("Error during metadata extraction: {}", ex.getMessage(), ex);
            OdpsSQLMetadataError error = new OdpsSQLMetadataError(0, 0, message);
            return OdpsSQLMetadataResult.withErrors(Collections.singletonList(error));
        }
    }
}
