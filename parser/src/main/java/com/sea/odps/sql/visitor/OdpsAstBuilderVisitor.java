...
import com.sea.odps.sql.visitor.core.SQLVisitor;
 ...
 * </ul>
 */
public class OdpsAstBuilderVisitor extends OdpsParserBaseVisitor<ASTNode> implements SQLVisitor {
     ...
     * @return ProjectionsSegment 对象，包含所有投影项的集合
     */
    private ASTNode visitSelectList(
            final OdpsParser.SelectListContext ctx, final boolean distinct) {
            ...
            if (null != ctx.member && operand instanceof ColumnSegment) {
                ...
                String raw =
                        null == ownerExpression
                                ? memberIdentifier
                                : ownerExpression + "." + memberIdentifier;
    ...
    public ASTNode visitPostSelectClauses(final OdpsParser.PostSelectClausesContext ctx) {
        ...
        if (null != ctx.limitClause()) {
            limitSegment =
                    ((PostSelectResult) visit(ctx.limitClause())).getLimitSegment().orElse(null);
        }
    ...
    private JoinTableSegment createNestedJoin(
        ...
        if (ctx.commonCols != null && !ctx.commonCols.isEmpty()) {
            List<ColumnSegment> usingColumns =
                    ctx.commonCols.stream()
                            .map(this::createColumnSegment)
    ...
    public ASTNode visitSubQuerySource(final OdpsParser.SubQuerySourceContext ctx) {
        ...
        if (ctx.subQueryExpression() != null) {
            ...
            subquerySegment =
                    new SubquerySegment(getStartIndex(ctx), getStopIndex(ctx), selectStatement);
        }
    ...
    private ColumnSegment createColumnSegment(final ParserRuleContext ctx, final String rawText) {
        ...
        ColumnSegment result =
                new ColumnSegment(
                        getStartIndex(ctx), getStopIndex(ctx), new IdentifierValue(columnText));
        if (ownerText != null && !ownerText.isEmpty()) {
            int ownerStart = getStartIndex(ctx);
            OwnerSegment ownerSegment =
                    new OwnerSegment(
                            ownerStart,
                            ownerStart + ownerText.length() - 1,
                            new IdentifierValue(ownerText));
    ...
    private PaginationValueSegment createPaginationValueSegment(
        ...
        try {
            ...
            return new NumberLiteralPaginationValueSegment(
                    getStartIndex(ctx), getStopIndex(ctx), value);
    ...
    private SelectStatement getLeftSelectStatement(final ParserRuleContext ctx) {
        if (ctx instanceof OdpsParser.QueryExpressionContext) {
            ...
            OdpsParser.SelectQueryStatementContext sqsCtx =
                    (OdpsParser.SelectQueryStatementContext) ctx;
            if (sqsCtx.q != null) {
    ...
    public ASTNode visitWithClause(final OdpsParser.WithClauseContext ctx) {
        ...
        if (null != ctx.branches && !ctx.branches.isEmpty()) {
            for (OdpsParser.CteStatementContext cteContext : ctx.branches) {
                CommonTableExpressionSegment cteSegment =
                        (CommonTableExpressionSegment) visit(cteContext);
                if (null != cteSegment) {
        ...
        WithSegment withSegment =
                new WithSegment(getStartIndex(ctx), getStopIndex(ctx), cteSegments);
        // 提取 WITH 子句附近的注释
        CommentSegment comment = extractCommentNearNode(ctx);
        if (comment != null) {
    ...
    public ASTNode visitCteStatement(final OdpsParser.CteStatementContext ctx) {
        ...
        if (ctx.queryExp != null) {
            ...
            if (selectStatement != null) {
                subquery =
                        new SubquerySegment(
                                getStartIndex(ctx.queryExp),
                                getStopIndex(ctx.queryExp),
                                selectStatement);
            }
        } else if (ctx.exp != null) {
    ...
    public ASTNode visitFunction(final OdpsParser.FunctionContext ctx) {
        ...
        // 普通函数，解析函数参数并创建 SimpleFunctionSegment
        String functionName = null;
        if (ctx.name != null) {
            functionName = getOriginalText(ctx.name);
        }
        ...
        if (ctx.arg != null) {
            for (OdpsParser.FunctionArgumentContext argCtx : ctx.arg) {
                ...
                if (argCtx.s != null) {
                    ...
                    // 函数引用作为参数，递归解析
                    arg = (ExpressionSegment) visit(argCtx.f);
                } else if (argCtx.l != null) {
                    // lambda 表达式暂不支持，解析为字面量表达式
                    arg = createLiteralExpression(argCtx.l);
                }
        ...
        return new SimpleFunctionSegment(
                getStartIndex(ctx), getStopIndex(ctx), functionName, arguments, originalText);
    }
    ...
    private void extractComments(
            final ParserRuleContext ctx, final AbstractSQLStatement statement) {
        ...
        for (Token token : allTokens) {
            ...
            if (commentText != null && !commentText.trim().isEmpty()) {
                statement.addCommentSegment(
                        new CommentSegment(
                                commentText, token.getStartIndex(), token.getStopIndex()));
            }
        }
    }
    ...
    private CommentSegment extractCommentNearNode(final ParserRuleContext ctx) {
        ...
        return extractCommentFromTokens(
                commonTokenStream.getHiddenTokensToLeft(
                        ctx.getStart().getTokenIndex(), COMMENT_CHANNEL),
                false);
    }
