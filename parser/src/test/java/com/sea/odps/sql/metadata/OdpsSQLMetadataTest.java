...
/** {@link com.sea.odps.sql.metadata.OdpsSQLMetadataEntrypoint} 测试。 */
public class OdpsSQLMetadataTest extends TestCase {
    ...
    public void testValidateSelectWithSubqueryAndFunctions() {
        ...
        com.sea.odps.sql.metadata.model.core.OdpsSQLMetadata metadata =
                result.getMetadata().orElseThrow(AssertionError::new);
        // 主代码会递归提取子查询内部的表，所以包括：
        // 1. 子查询 t（别名）
        // 2. 子查询 s（别名）
        // 3. ods.fact_sales（子查询 t 内部的表）
        // 4. ods.user_score（子查询 s 内部的表）
        assertTrue("应该有至少2个表（子查询别名）", metadata.getTables().size() >= 2);
        ...
        Collection<FieldMetadata> fields = metadata.getFields();
        // 主代码会递归提取子查询内部的字段，所以包括：
        // 主查询的3个字段：t.country, total_amount, max_score
        // 子查询 t 的字段：user_id, country, amount
        // 子查询 s 的字段：user_id, score
        assertTrue("应该有至少3个字段（主查询字段）", fields.size() >= 3);
    ...
    public void testWindowFunctionComplex() {
        ...
        long inlineSpecs =
                windowFunctions.stream()
                        .filter(
                                f ->
                                        f.getWindowNameRef() == null
                                                && !f.getPartitionColumns().isEmpty())
                        .count();
