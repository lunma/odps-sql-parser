...
public class CommentExtractionTest extends TestCase {
    ...
    public void testTableComment() {
        ...
        com.sea.odps.sql.metadata.model.core.OdpsSQLMetadata metadata =
                result.getMetadata().orElseThrow(AssertionError::new);
        ...
        for (CommentMetadata comment : comments) {
            System.out.println(
                    "  类型: " + comment.getTargetType() + ", 文本: " + comment.getText().trim());
        }
        ...
        Optional<CommentMetadata> tableComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.TABLE)
                        .findFirst();
    ...
    public void testFieldComment() {
        ...
        Optional<CommentMetadata> idComment =
                comments.stream()
                        .filter(
                                c ->
                                        c.getTargetType() == CommentTargetType.FIELD
                                                && c.getText().contains("用户ID"))
                        .findFirst();
    ...
    public void testWhereComment() {
        ...
        Optional<CommentMetadata> whereComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.WHERE)
                        .findFirst();
    ...
    public void testGroupByComment() {
        ...
        Optional<CommentMetadata> groupByComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.GROUP_BY)
                        .findFirst();
    ...
    public void testHavingComment() {
        ...
        Optional<CommentMetadata> havingComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.HAVING)
                        .findFirst();
    ...
    public void testOrderByComment() {
        ...
        Optional<CommentMetadata> orderByComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.ORDER_BY)
                        .findFirst();
    ...
    public void testLimitComment() {
        String sql =
                "SELECT id, name FROM ods.user_table ORDER BY id LIMIT 10 -- 限制返回 10 条\nOFFSET 5;";
        OdpsSQLMetadataResult result = validator.result(sql);
        assertTrue("SQL 应该被支持", result.isSupported());
        assertTrue("SQL 应该有效", result.isValid());
        ...
        Optional<CommentMetadata> limitComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.LIMIT)
                        .findFirst();
    ...
    public void testWithCteComment() {
        ...
        Optional<CommentMetadata> cteComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.CTE)
                        .findFirst();
    ...
    public void testHintComment() {
        ...
        Optional<CommentMetadata> hintComment =
                comments.stream()
                        .filter(c -> c.getTargetType() == CommentTargetType.HINT)
                        .findFirst();
    ...
    public void testCommentAssociation() {
        ...
        assertTrue(
                ...
                fieldMetadata
                        .getAliasOptional()
                        .map(alias -> "user_id".equals(alias))
                        .orElse(false));
        ...
        Optional<CommentMetadata> tableComment =
                comments.stream()
                        .filter(
                                c ->
                                        c.getTargetType() == CommentTargetType.TABLE
                                                && c.getText().contains("用户表"))
                        .findFirst();
    ...
    public void testPrintComments() {
        ...
        for (CommentMetadata comment : comments) {
            ...
            System.out.println(
                    "  位置: [" + comment.getStartIndex() + ", " + comment.getStopIndex() + "]");
